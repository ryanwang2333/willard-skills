...
Usage:
  python3 fetch.py <url> [max_chars] [--stealth] [--no-proxy]
...
# 本机系统代理:Windows 系统代理指向 127.0.0.1:7890(CordCCore.exe),curl/httpx 默认不读,
# 这里显式带上才能访问 google 等被墙站。环境变量 FETCH_PROXY 可覆盖,`--no-proxy` 可关闭。
PROXY = os.environ.get("FETCH_PROXY", "http://127.0.0.1:7890")

# 无头浏览器用系统 Edge:本机安全策略禁止执行 ms-playwright 的 chromium(launch 报 spawn UNKNOWN)。
...
def fetch_fast(url, max_chars=30000, timeout=15):
    ...
    try:
        ...
        if PROXY is None:
            raise
        # 直连失败(多为被墙站),自动改走本机系统代理重试一次
        page = Fetcher().get(url, timeout=timeout, stealthy_headers=True, proxy=PROXY)
...
def fetch_stealth(url, max_chars=30000, timeout=30000):
    ...
    Uses the system Edge via executable_path because playwright's bundled
    chromium is blocked by the local security policy.
    """
    from scrapling.fetchers import StealthyFetcher
    _suppress_scrapling_logs()

    fetch_kwargs = {"headless": True, "network_idle": True, "timeout": timeout}
    if EDGE_PATH:
        fetch_kwargs["executable_path"] = EDGE_PATH
    # 不显式传 proxy:Edge 自行走系统代理(127.0.0.1:7890),被墙站由代理转发、
    # 国内站按代理分流直连;显式指定代理反而会让 sspai 等直连站连不上。
...
def main():
    # Windows 控制台默认 GBK 编码,会把 UTF-8 中文打成乱码
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8")

    if len(sys.argv) < 2:
        print(
            "Usage: python3 fetch.py <url> [max_chars] [--stealth] [--no-proxy]\n"
            "\n"
            "Options:\n"
